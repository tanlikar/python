...
def plotGraph (name, month, date):
    df = pd.read_csv('C:/sensor data/' + name + '1.csv')
...
def plotDate(month, date):
    for x in sensor:
        ...
        plt.savefig('C:/sensor data/sensor graph/' + x + '.png')
        plt.show()
...
def plotDateOverall(month, date):
    ...
    plt.legend()
    plt.ylabel('')
    plt.savefig('C:/sensor data/sensor graph/all.png')
    plt.show()
    
        
plotDate('Nov', '7')
plotDateOverall('Nov', '7')
